  ...
  module FastServer
    class Interface
      ...
      def call(env)
        ...
        if request.path =~ %r{\A/(\.well-known/.*)}
          if certificate = ::TrackCertificate.find_by_verification_path(Regexp.last_match(1))
        ...
        elsif request.path =~ %r{\A/img/([a-z0-9\-]+)/([a-z0-9\-]+)}i
        ...
        if request.path =~ %r{\A/([a-z0-9\-]+)/([a-z0-9\-]+)}i
          server_token = Regexp.last_match(1)
