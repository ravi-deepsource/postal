module Postal
  class Worker
    ...
    def work
      ...
      loop do
        if @exit && @running_jobs.empty?
          ...
          logger.info 'Exit requested but job is running. Waiting for job to finish.' if exit_checks == 0
          sleep 60
          exit_checks += 1
        else
          manage_ip_queues
    ...
    def receive_job(delivery_info, _properties, body)
      ...
      if message && message['class_name']
        ...
        begin
          ...
          Raven.capture_exception(e, extra: { job_id: message['id'] }) if defined?(Raven)
        ensure
    ...
    def manage_ip_queues
      ...
      unwanted_queues.each do |id|
        ...
        @ip_to_id_mapping.each do |_ip, _id|
          ip_addresses_to_clear << _ip if id == _id
        end
    ...
    def set_process_name
      ...
      $0 = if @running_jobs.empty?
             "#{prefix} (idle)"
           else
             "#{prefix} (running #{@running_jobs.join(', ')})"
           end
    end
