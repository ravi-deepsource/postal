Rails.application.routes.draw do
  scope 'org/:org_permalink', as: 'organization' do
    resources :domains, only: %i[index new create destroy] do
      match :verify, on: :member, via: %i[get post]
      get :setup, on: :member
  ...
  match 'verify' => 'user#verify', :via => %i[get post]
  get 'signup/:invite_token' => 'user#new', :as => 'new_signup'
  post 'signup' => 'user#create'
  match 'join/:token' => 'user#join', :via => %i[get post delete], :as => 'join'
  get 'login' => 'sessions#new'
  post 'login' => 'sessions#create'
  get 'login/token' => 'sessions#create_with_token'
  delete 'logout' => 'sessions#destroy'
  match 'login/reset' => 'sessions#begin_password_reset', :via => %i[get post]
  match 'login/reset/:token' => 'sessions#finish_password_reset', :via => %i[get post]
