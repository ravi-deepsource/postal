class OrganizationsController < ApplicationController
  ...
  before_action :require_organization_admin, only: %i[edit update delete destroy]
  ...
  def index
    if current_user.admin?
      ...
      redirect_to organization_root_path(@organizations.first) if @organizations.size == 1 && params[:nrd].nil?
    end
  end
  ...
  def organization
    if %i[edit update delete destroy].include?(action_name.to_sym)
