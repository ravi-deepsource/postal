class ServersController < ApplicationController
  ...
  before_action :require_organization_admin, only: %i[new create delete destroy]
  before_action :admin_required, only: %i[advanced suspend unsuspend]
  ...
  def show
    if @server.created_at < 48.hours.ago
      @graph_type = :daily
      graph_data = @server.message_db.statistics.get(:daily, %i[incoming outgoing bounces], Time.now, 30)
    elsif @server.created_at < 24.hours.ago
      @graph_type = :hourly
      graph_data = @server.message_db.statistics.get(:hourly, %i[incoming outgoing bounces], Time.now, 48)
    else
      @graph_type = :hourly
      graph_data = @server.message_db.statistics.get(:hourly, %i[incoming outgoing bounces], Time.now, 24)
    end
  ...
  def update
    extra_params = %i[spam_threshold spam_failure_threshold postmaster_address]
    if current_user.admin?
      extra_params += %i[send_limit allow_sender log_smtp_data outbound_spam_threshold message_retention_days raw_message_retention_days raw_message_retention_size]
    end
