...
class Server < ApplicationRecord
  ...
  after_create do
    message_db.provisioner.provision unless provision_database == false
  end

  after_commit(on: :destroy) do
    message_db.provisioner.drop unless provision_database == false
  end
  ...
  def message_rate
    @message_rate ||= message_db.live_stats.total(60, types: %i[incoming outgoing]) / 60.0
  end
  ...
  def bounce_rate
    @bounce_rate ||= begin
      ...
      total_outgoing = 0.0
      total_bounces = 0.0
      message_db.statistics.get(:daily, %i[outgoing bounces], time, 30).each do |_date, stat|
  ...
  def find_authenticated_domain_from_headers(headers)
    ...
    header_to_check.each do |header_name|
      values = if headers[header_name].is_a?(Array)
                 headers[header_name]
               else
      ...
      return authenticated_domains.first if authenticated_domains.size == values.size
    end
    nil
  end
  ...
  def ip_pool_for_message(message)
    if message.scope == 'outgoing'
      [self, organization].each do |scope|
        ...
        rules.each do |rule|
          return rule.ip_pool if rule.apply_to_message?(message)
        end
      end
      ip_pool
    end
  ...
  def self.send_send_limit_notifications
    %i[approaching exceeded].each_with_object({}) do |type, hash|
      hash[type] = 0
